...
/* harmony import */ var _modules_formsjson__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./modules/formsjson */ "./src/js/modules/formsjson.js");
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_sliders__WEBPACK_IMPORTED_MODULE_6__["sliders"])('.main-slider-item', 'vertical');
  Object(_modules_sliders__WEBPACK_IMPORTED_MODULE_6__["sliders"])('.feedback-slider-item', 'horizontal', '.main-prev-btn', '.main-next-btn');
  Object(_modules_formsjson__WEBPACK_IMPORTED_MODULE_2__["default"])(state);
  Object(_modules_mask__WEBPACK_IMPORTED_MODULE_3__["mask"])('[name="phone"]');
  Object(_modules_checkTextInputs__WEBPACK_IMPORTED_MODULE_1__["checkTextInputs"])('[name="name"]');
  Object(_modules_checkTextInputs__WEBPACK_IMPORTED_MODULE_1__["checkTextInputs"])('[name="message"]');
  Object(_modules_showMoreStyles__WEBPACK_IMPORTED_MODULE_5__["default"])('.button-styles', '.styles-2');
  Object(_modules_calc__WEBPACK_IMPORTED_MODULE_0__["calc"])("#size", "#material", "#options", ".promocode", ".calc-price", state);
...
//     });
// };
var acconrdion = function acconrdion(triggerSelector, itemsSelector) {
...
/* harmony default export */ __webpack_exports__["default"] = (acconrdion);
...
var burger = function burger(menuSelector, burgerSelector) {
  ...
  window.addEventListener('resize', function () {
    if (window.screen.availWidth > 992) {
      // если ширина экрана изменится и станет > 992px .
      menuElem.style.display = 'none';
    }
  });
};
...
var calc = function calc(size, material, options, promocode, result, state) {
  ...
  var calcFunc = function calcFunc() {
    ...
    if (sizeBlock.value === "" || materialBlock.value === "") {
      resultBlock.textContent = "Пожалуйста, введите размер и материал картины";
...
/*!*******************************************!*\
  !*** ./src/js/modules/checkTextInputs.js ***!
  \*******************************************/
/*! exports provided: checkTextInputs */
/***/ (function(module, __webpack_exports__, __webpack_require__) {
  ...
  txtInputs.forEach(function (input) {
    input.addEventListener('keypress', function (e) {
      if (e.key.match(/[^а-яё 0-9]/ig)) {
        // символы а-яё 0-9 во всей строке(i), и вне зависимости от регистра(g)
        e.preventDefault();
      }
    });
  });
...
var dragDrop = function dragDrop() {
  ...
  // * - элементы перетаскиваемые внутри дом элемента
  ...
  function highlight(item) {
    // подсвечивание элемента при drag (претаскивании фото)
    item.closest('.file_upload').style.border = "5px solid yellow";
    item.closest('.file_upload').style.backgroundColor = "rgba(0,0,0, .7)";
  }

  function unhighlight(item) {
    // убрать подсвечивание элемента .
...
var filter = function filter() {
  ...
  var typeFilter = function typeFilter(markType) {
    markAll.forEach(function (item) {
      ...
      item.classList.remove('animated', 'tada');
    });
    no.style.display = 'none';
    no.classList.remove('animated', 'tada');
    ...
    if (markType) {
      ...
      no.classList.add('animated', 'tada');
    }
  };
...
var forms = function forms(state) {
  ...
  var path = {
    designer: 'https://winmarket-1-default-rtdb.europe-west1.firebasedatabase.app/design.json',
    question: 'https://winmarket-1-default-rtdb.europe-west1.firebasedatabase.app/quest.json'
  };
  ...
  upload.forEach(function (item) {
    item.addEventListener('input', function () {
      ...
      item.previousElementSibling.textContent = name; // предыдущий элемент от нашего нода
    });
  });
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      if (item.getAttribute('data-calc') === "flag") {
        // добавил сумму к заказу. форма отправляет только id и сумму.
        for (var key in state) {
          formData.append(key, state[key]);
        }
      }

      var object = {}; // formData => json
      ...
      console.log(api);
      postData(api, jsonFile).then(function (res) {
        console.log(res);
        ...
        setTimeout(function () {
          ...
          item.classList.remove('fadeOutUp'); // класс отвечает за скрытие формы

          item.classList.add('fadeInUp'); // класс отвечает за скрытие формы
        }, 5000);
      });
    });
  });
...
var mask = function mask(selector) {
  var setCursorPosition = function setCursorPosition(pos, elem) {
    ...
    if (elem.setSelectionRange) {
      // ручной полифил на поддержку браузером метода elem.setSelectionRange(posStart, posEnd)
  ...
  function createMask(event) {
    ...
    if (def.length >= val.length) {
      // +7 в matrix - цифры. Данное устовие не дает их удалить с input .
      val = def;
    }
...
var modals = function modals() {
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    var triggers = document.querySelectorAll(triggerSelector),
        ...
        // установить обязательно к модальному окну.
    scroll = calcScroll(); // узнаем ширину полосы прокрутки .

    triggers.forEach(function (trigger) {
      trigger.addEventListener('click', function (e) {
        if (e.target) {
          // обязательно, чтобы при клике на ссылку не перезагружалась страница
          e.preventDefault();
        }
        ...
        document.body.style.marginRight = "".concat(scroll, "px"); // устанавливаем внешний отступ справа от элемента
  ...
  function calcScroll() {
    // функция нужна чтобы при открытии мод. окон не прыгало изображение при исчезновении полосы прокрутки .
    var div = document.createElement('div'); // создаем элемент, затем удаляем .
    ...
    var scrollWidth = div.offsetWidth - div.clientWidth; // (ширина вместе со скроллом) - (ширина без скролла)
  ...
  function openModalByScroll(selector) {
    window.addEventListener('scroll', function () {
      //если 
      if (btnPressed === false && window.pageYOffset + document.documentElement.clientHeight >= document.documentElement.scrollHeight) {
        document.querySelector(selector).click(); //window.removeEventListener('scroll', openModalByScroll);
      }
    });
  }
  ...
  openModalByScroll('.infinite'); //showModalByTime('.popup-consultation', 60000);
}; // const modals = () => {
...
var pictureSize = function pictureSize(imgSelector) {
  ...
  function showImg(block) {
    ...
    img.src = img.src.slice(0, -4) + '-1.png'; // скрываем все элементы кроме эл. с классом size-hit
  ...
  function hideImg(block) {
    ...
    img.src = img.src.slice(0, -6) + '.png'; // показываем все элементы кроме эл. с классом size-hit
...
var scrolling = function scrolling(upSelector) {
  ...
  var links = document.querySelectorAll('[href^="#"]'); // все ссылки котор. начинаются с # т.е. локальные .
  ...
  links.forEach(function (link) {
    link.addEventListener('click', function (event) {
      ...
      var widthTop = document.documentElement.scrollTop,
          ...
          startTime = null; // .getBoundingClientRect() дает доступ получить доступ к свойствам.
...
/* harmony import */ var core_js_modules_web_dom_collections_for_each__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! core-js/modules/web.dom-collections.for-each */ "./node_modules/core-js/modules/web.dom-collections.for-each.js");
/* harmony import */ var core_js_modules_web_dom_collections_for_each__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(core_js_modules_web_dom_collections_for_each__WEBPACK_IMPORTED_MODULE_2__);
/* harmony import */ var _services_resquests__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../services/resquests */ "./src/js/services/resquests.js");
...
var showMoreStyles = function showMoreStyles(trigger, styles) {
  ...
  var wrapper = document.querySelector('#styles .row'); //  пример когда нужно просто показать скрытые элементы из страницы
  // cards.forEach(card => {
  ...
  btn.addEventListener('click', function () {
    Object(_services_resquests__WEBPACK_IMPORTED_MODULE_3__["getResource"])('https://winmarket-1-default-rtdb.europe-west1.firebasedatabase.app/styles.json').then(function (res) {
...
var sliders = function sliders(slidesSelector, dir, prev, next) {
  ...
  function activateAutoAnimation() {
    if (dir === 'vertical') {
    ...
    while (1) {
      switch (_context2.prev = _context2.next) {
        ...
        case 2:
          ...
          throw new Error("Corld not fetch ".concat(url, ", status: ").concat(res.status));
